...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      'packages': fileURLToPath(new URL('./packages/index', import.meta.url))
    }
  },
  css: {
    preprocessorOptions: {
  ...
  build: {
    ...
    rollupOptions: { // 自定义底层的Rollup打包配置
      ...
      output: {
        name: 'NightSoulUI', // 对于输出格式为 iife | umd 的 bundle 来说，若想要使用全局变量名来表示你的 bundle 时，该选项是必要的。同一页面上的其他脚本可以使用这个变量名来访问你的 bundle 输出
        /*
          output.format: 
